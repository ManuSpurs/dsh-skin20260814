...
- **选择图片 / Choose image** — pick a local image; the **original file is
  stored as-is** (IndexedDB, no recompression) and shown at full quality,
...
The wallpaper picture is stored at **original quality** in IndexedDB (a raw
Blob — the browser's localStorage quota cannot hold a full-size image);
slider values and the skin choice stay in `localStorage`
(`dsh-skin:skin`, `dsh-skin:wallpaper-opacity`, `dsh-skin:wallpaper-blur`).
